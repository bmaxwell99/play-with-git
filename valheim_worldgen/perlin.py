"""
Reimplementation of Unity's Mathf.PerlinNoise.

Ken Perlin's improved noise (2002) with absolute-value inputs,
doubled permutation indexing, and Unity's custom output normalization.
Test vector: noise(0, 0) == 0.4652731 (approx).

Ported from BjarkeCK/ValheimSeedFinder and RyanDMcAfee/ValheimBakaLoader.
"""
...
PERM = _SOURCE + _SOURCE  # doubled to 512
...
def noise(x: float, y: float) -> float:
    """Matches Unity Mathf.PerlinNoise(x, y). Output approximately in [0, 1]."""
    x = abs(x)
    y = abs(y)
    ...
    a = PERM[cell_x] + cell_y
    b = PERM[cell_x + 1] + cell_y
